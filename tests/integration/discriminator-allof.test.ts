...
describe('Discriminator allOf integration', () => {
  ...
  it('generated contracts contain discriminator types', () => {
    expect(contracts).toContain('"$type": string');
    expect(contracts).toContain('export type Cat = Pet & {');
    expect(contracts).toContain('export type Dog = Pet & {');
    expect(contracts).toContain('PetVariant = Cat | Dog');
  });

  // 3. Response types use PetVariant union
  it('response types use PetVariant instead of Pet', () => {
    expect(contracts).toContain('PetVariant = Cat | Dog');
    expect(contracts).toContain('GetPetsResponse');
    expect(contracts).toContain('GetPetsIdResponse');
