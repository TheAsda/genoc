// Feature coverage: 3.1-#1 (string), 3.1-#2 (number), 3.1-#3 (integer), 3.1-#4 (boolean),
// 3.1-#5 (array), 3.1-#6 (object), 3.1-#7 (null), 3.1-#8 (type arrays),
// 3.1-#9 (allOf), 3.1-#10 (oneOf), 3.1-#11 (anyOf), 3.1-#12 (discriminator),
// 3.1-#13 (enum), 3.1-#14 (const), 3.1-#15 (default), 3.1-#16 (description),
// 3.1-#17 (readOnly), 3.1-#18 (writeOnly), 3.1-#19 (deprecated), 3.1-#20 (format),
// 3.1-#21 (additionalProperties), 3.1-#22 (required),
// 3.1-#23 (minItems), 3.1-#24 (maxItems), 3.1-#25 (minLength), 3.1-#26 (maxLength),
// 3.1-#27 (pattern), 3.1-#28 (minimum), 3.1-#29 (maximum),
// 3.1-#30 (exclusiveMinimum), 3.1-#31 (exclusiveMaximum), 3.1-#32 (examples)

/**
 * Validation Tests — OpenAPI 3.1 Data Types & Schema Keywords
 *
 * Tests every feature from the "Data Types" (3.1-#1-#8) and "Schema Keywords"
 * (3.1-#9-#32) sections of the feature enumeration.
 *
 * Tier 1 (3.1-#1-#22): generateContracts + string matching on TypeScript output
 * Tier 2 (3.1-#23-#29): verify no crash + constraint NOT emitted in output
 * Tier 3 (3.1-#30-#32): version-specific normalization comparison (3.0 vs 3.1)
 */
import { describe, expect, it } from 'vitest';
import { parse as parseYaml } from 'yaml';

import { generateClient as generateClientStrings } from '../../src/generator/client-generator.js';
import { generateContracts } from '../../src/generator/contracts-generator.js';
import { RefResolver } from '../../src/parser/ref-resolver.js';
import type { GeneratorConfig } from '../../src/types/client.js';
import type { OpenAPIDocument } from '../../src/types/openapi.js';

// ── Helpers ────────────────────────────────────────────────────────────────

function generateFromYaml(yaml: string): string {
  const doc = parseYaml(yaml) as OpenAPIDocument;
  const resolver = new RefResolver(doc);
  return generateContracts(doc, resolver);
}

function generateClientFromYaml(yaml: string): { contracts: string; client: string } {
  const doc = parseYaml(yaml) as OpenAPIDocument;
  const config: GeneratorConfig = { input: 'test.yaml', outputDir: '/tmp/test' };
  return generateClientStrings(doc, config);
}

const BASE_SPEC = (schemasYaml: string) => `
openapi: "3.1.0"
info: { title: Test, version: "1.0.0" }
paths: {}
components:
  schemas:
${schemasYaml
  .split('\n')
  .map((l) => '    ' + l)
  .join('\n')}`;

// ── Data Types (3.1-#1-#8) — Tier 1 ───────────────────────────────────────

describe('OpenAPI 3.1 — Data Types (3.1-#1-#8)', () => {
  // 3.1-#1: string
  it('3.1-#1: maps string type to TypeScript string', () => {
    const result = generateFromYaml(BASE_SPEC('Status: { type: string }'));
    expect(result).toContain('export type Status = string;');
  });

  // 3.1-#2: number
  it('3.1-#2: maps number type to TypeScript number', () => {
    const result = generateFromYaml(BASE_SPEC('Price: { type: number }'));
    expect(result).toContain('export type Price = number;');
  });

  // 3.1-#3: integer
  it('3.1-#3: maps integer type to TypeScript number', () => {
    const result = generateFromYaml(BASE_SPEC('Count: { type: integer }'));
    expect(result).toContain('export type Count = number;');
  });

  // 3.1-#4: boolean
  it('3.1-#4: maps boolean type to TypeScript boolean', () => {
    const result = generateFromYaml(BASE_SPEC('Active: { type: boolean }'));
    expect(result).toContain('export type Active = boolean;');
  });

  // 3.1-#5: array
  it('3.1-#5: maps array with items to TypeScript T[]', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Tags:
        type: array
        items:
          type: string
    `)
    );
    expect(result).toContain('export type Tags = string[];');
  });

  // 3.1-#6: object
  it('3.1-#6: maps object with properties to TypeScript interface-like type', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      User:
        type: object
        properties:
          name: { type: string }
          age: { type: integer }
        required: [name]
    `)
    );
    expect(result).toContain('export type User = {');
    expect(result).toContain('name: string;');
    expect(result).toContain('age?: number;');
  });

  // 3.1-#7: null
  it('3.1-#7: maps null type to TypeScript null', () => {
    const result = generateFromYaml(BASE_SPEC("Nothing: { type: 'null' }"));
    expect(result).toContain('export type Nothing = null;');
  });

  // 3.1-#8: type arrays (nullable via type array)
  describe('3.1-#8: type arrays', () => {
    it('maps type: [string, null] to string | null', () => {
      const result = generateFromYaml(
        BASE_SPEC(`
        NullableName:
          type: [string, "null"]
      `)
      );
      expect(result).toContain('export type NullableName = string | null;');
    });

    it('maps type: [string, number] to string (first non-null type used)', () => {
      const result = generateFromYaml(
        BASE_SPEC(`
      Multi:
        type: [string, number]
    `)
      );
      expect(result).toContain('export type Multi = string;');
    });

    it('maps type: [integer, null] to number | null', () => {
      const result = generateFromYaml(
        BASE_SPEC(`
        NullableInt:
          type: [integer, "null"]
      `)
      );
      expect(result).toContain('export type NullableInt = number | null;');
    });
  });
});

// ── Composition & Discriminator (3.1-#9-#12) — Inseparable Group, Tier 1 ──

describe('OpenAPI 3.1 — Composition & Discriminator (3.1-#9-#12)', () => {
  const DISCRIMINATOR_SPEC = `
openapi: "3.1.0"
info: { title: Test, version: "1.0.0" }
paths: {}
components:
  schemas:
    Pet:
      type: object
      properties:
        petType:
          type: string
      required: [petType]
      discriminator:
        propertyName: petType
        mapping:
          cat: "#/components/schemas/Cat"
          dog: "#/components/schemas/Dog"
    Cat:
      allOf:
        - $ref: "#/components/schemas/Pet"
        - type: object
          properties:
            meow:
              type: boolean
    Dog:
      allOf:
        - $ref: "#/components/schemas/Pet"
        - type: object
          properties:
            bark:
              type: boolean
    PetVariant:
      oneOf:
        - $ref: "#/components/schemas/Cat"
        - $ref: "#/components/schemas/Dog"
      discriminator:
        propertyName: petType
        mapping:
          cat: "#/components/schemas/Cat"
          dog: "#/components/schemas/Dog"
    MixedUnion:
      anyOf:
        - type: string
        - type: number
  `;

  // 3.1-#9: allOf
  it('3.1-#9: generates intersection type (&) for allOf', () => {
    const result = generateFromYaml(DISCRIMINATOR_SPEC);
    expect(result).toContain('export type Cat = Pet &');
    expect(result).toContain('export type Dog = Pet &');
  });

  // 3.1-#10: oneOf
  it('3.1-#10: generates union type (|) for oneOf without discriminator', () => {
    const result = generateFromYaml(`
openapi: "3.1.0"
info: { title: Test, version: "1.0.0" }
paths: {}
components:
  schemas:
    Cat: { type: object, properties: { meow: { type: boolean } } }
    Dog: { type: object, properties: { bark: { type: boolean } } }
    Pet:
      oneOf:
        - $ref: "#/components/schemas/Cat"
        - $ref: "#/components/schemas/Dog"
    `);
    expect(result).toContain('export type Pet = Cat | Dog;');
  });

  // 3.1-#11: anyOf
  it('3.1-#11: generates union type (|) for anyOf', () => {
    const result = generateFromYaml(DISCRIMINATOR_SPEC);
    expect(result).toContain('export type MixedUnion = string | number;');
  });

  // 3.1-#12: discriminator
  it('3.1-#12: generates discriminated union with propertyName and mapping', () => {
    const result = generateFromYaml(DISCRIMINATOR_SPEC);
    expect(result).toContain('petType');
    expect(result).toContain('export type PetVariant =');
  });
});

// ── Enum & Const (3.1-#13-#14) — Tier 1 ───────────────────────────────────

describe('OpenAPI 3.1 — Enum & Const (3.1-#13-#14)', () => {
  // 3.1-#13: enum
  it('3.1-#13: generates string literal union for string enum', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Status:
        type: string
        enum: [active, inactive, pending]
    `)
    );
    expect(result).toContain("'active' | 'inactive' | 'pending'");
  });

  it('3.1-#13: generates number literal union for number enum', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Priority:
        type: integer
        enum: [1, 2, 3]
    `)
    );
    expect(result).toContain('1 | 2 | 3');
  });

  it('3.1-#13: generates boolean literal union for boolean enum', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Flag:
        type: boolean
        enum: [true, false]
    `)
    );
    expect(result).toContain('true | false');
  });

  it('3.1-#13: generates null-including union for enum with null', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      NullableStatus:
        type: string
        enum: [active, inactive, null]
    `)
    );
    expect(result).toContain("'active' | 'inactive' | null");
  });

  // 3.1-#14: const
  it('3.1-#14: does not crash when const is present with type — type is emitted, const ignored', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      FixedVersion:
        type: string
        const: "1.0.0"
    `)
    );
    expect(result).toContain('export type FixedVersion = string;');
  });

  it('3.1-#14: produces unknown for const without type (no type handler)', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      JustConst:
        const: "hello"
    `)
    );
    expect(result).toContain('export type JustConst = unknown;');
  });
});

// ── Default & Description (3.1-#15-#16) — Tier 1 ──────────────────────────

describe('OpenAPI 3.1 — Default & Description (3.1-#15-#16)', () => {
  // 3.1-#15: default
  it('3.1-#15: does not crash with default value — property is correctly typed', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Config:
        type: object
        properties:
          name: { type: string }
          timeout: { type: integer, default: 30 }
        required: [name]
    `)
    );
    expect(result).toContain('name: string;');
    expect(result).toContain('timeout?: number;');
  });

  // 3.1-#16: description
  it('3.1-#16: generates JSDoc comment from description on schema', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Product:
        type: object
        description: "A product in the catalog"
        properties:
          name: { type: string }
    `)
    );
    expect(result).toContain('/** A product in the catalog */');
    expect(result).toContain('export type Product =');
  });

  it('3.1-#16: generates JSDoc comment from description on individual type alias', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Email:
        type: string
        description: "A valid email address"
    `)
    );
    expect(result).toContain('/** A valid email address */');
    expect(result).toContain('export type Email = string;');
  });
});

// ── ReadOnly & WriteOnly (3.1-#17-#18) — Inseparable Pair, Tier 1 ─────────

describe('OpenAPI 3.1 — ReadOnly & WriteOnly (3.1-#17-#18)', () => {
  const READ_WRITE_SPEC = `
openapi: "3.1.0"
info: { title: Test, version: "1.0.0" }
paths:
  /items:
    post:
      operationId: createItem
      requestBody:
        required: true
        content:
          application/json:
            schema:
              type: object
              properties:
                name:
                  type: string
                id:
                  type: string
                  readOnly: true
                password:
                  type: string
                  writeOnly: true
              required: [name, id, password]
      responses:
        "201":
          description: Created
          content:
            application/json:
              schema:
                type: object
                properties:
                  name:
                    type: string
                  id:
                    type: string
                    readOnly: true
                  password:
                    type: string
                    writeOnly: true
                required: [name, id]
  `;

  // 3.1-#17: readOnly
  it('3.1-#17: excludes readOnly properties from request body type', () => {
    const { contracts } = generateClientFromYaml(READ_WRITE_SPEC);
    expect(contracts).toContain('PostItemsBody');
    expect(contracts).not.toMatch(/PostItemsBody = \{[^}]*id[^}]*\}/s);
  });

  it('3.1-#17: includes readOnly properties in response type', () => {
    const { contracts } = generateClientFromYaml(READ_WRITE_SPEC);
    expect(contracts).toContain('PostItemsResponse');
    expect(contracts).toMatch(/PostItemsResponse.*=.*\{[^}]*id/s);
  });

  // 3.1-#18: writeOnly
  it('3.1-#18: excludes writeOnly properties from response type', () => {
    const { contracts } = generateClientFromYaml(READ_WRITE_SPEC);
    expect(contracts).toContain('PostItemsResponse');
    expect(contracts).not.toMatch(/PostItemsResponse.*=.*\{[^}]*password/s);
  });

  it('3.1-#18: includes writeOnly properties in request body type', () => {
    const { contracts } = generateClientFromYaml(READ_WRITE_SPEC);
    expect(contracts).toContain('PostItemsBody');
    expect(contracts).toMatch(/PostItemsBody.*=.*\{[^}]*password/s);
  });
});

// ── Deprecated (3.1-#19) — Tier 1 ─────────────────────────────────────────

describe('OpenAPI 3.1 — Deprecated (3.1-#19)', () => {
  it('3.1-#19: includes @deprecated JSDoc for deprecated operations', () => {
    const { client } = generateClientFromYaml(`
openapi: "3.1.0"
info: { title: Test, version: "1.0.0" }
paths:
  /old:
    get:
      operationId: oldEndpoint
      deprecated: true
      responses:
        "200":
          description: OK
          content:
            application/json:
              schema:
                type: string
    `);
    expect(client).toContain('@deprecated');
  });

  it('3.1-#19: does not crash for schema with deprecated flag — deprecated is stored but not emitted in contracts', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      LegacyType:
        type: string
        deprecated: true
    `)
    );
    expect(result).toContain('export type LegacyType = string;');
  });
});

// ── Format (3.1-#20) — Tier 1 ─────────────────────────────────────────────

describe('OpenAPI 3.1 — Format (3.1-#20)', () => {
  it('3.1-#20: generates branded type for date-time format on string', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      CreatedAt:
        type: string
        format: date-time
    `)
    );
    expect(result).toContain('DateTimeString');
    expect(result).toContain('export type CreatedAt = DateTimeString;');
  });

  it('3.1-#20: generates branded type for uuid format on string', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      UserId:
        type: string
        format: uuid
    `)
    );
    expect(result).toContain('UuidString');
    expect(result).toContain('export type UserId = UuidString;');
  });

  it('3.1-#20: generates branded type definition with format tag', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Timestamp:
        type: string
        format: date-time
    `)
    );
    expect(result).toContain(
      "export type DateTimeString = string & { readonly __format?: 'date-time' };"
    );
  });

  it('3.1-#20: generates plain string for binary format (no branding)', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      FileData:
        type: string
        format: binary
    `)
    );
    expect(result).toContain('export type FileData = string;');
  });

  it('3.1-#20: generates plain string for byte format (no branding)', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Encoded:
        type: string
        format: byte
    `)
    );
    expect(result).toContain('export type Encoded = string;');
  });

  it('3.1-#20: generates branded type for numeric format', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Score:
        type: integer
        format: int32
    `)
    );
    expect(result).toContain('Int32Number');
    expect(result).toContain('export type Score = Int32Number;');
  });
});

// ── AdditionalProperties & Required (3.1-#21-#22) — Tier 1 ────────────────

describe('OpenAPI 3.1 — AdditionalProperties & Required (3.1-#21-#22)', () => {
  // 3.1-#21: additionalProperties
  it('3.1-#21: generates index signature for additionalProperties with type', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      StringMap:
        type: object
        additionalProperties: { type: string }
    `)
    );
    expect(result).toContain('[key: string]: string');
  });

  it('3.1-#21: generates Record for additionalProperties on inline object', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Container:
        type: object
        properties:
          metadata:
            type: object
            additionalProperties: { type: integer }
    `)
    );
    expect(result).toContain('Record<string, number>');
  });

  it('3.1-#21: generates [key: string]: unknown for additionalProperties: true', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      FreeForm:
        type: object
        properties:
          name: { type: string }
        additionalProperties: true
    `)
    );
    expect(result).toContain('[key: string]: unknown');
  });

  // 3.1-#22: required
  it('3.1-#22: marks required properties without ?, optional properties with ?', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Profile:
        type: object
        properties:
          username: { type: string }
          email: { type: string }
          bio: { type: string }
        required: [username, email]
    `)
    );
    expect(result).toContain('username: string;');
    expect(result).toContain('email: string;');
    expect(result).toContain('bio?: string;');
  });

  it('3.1-#22: all properties optional when required is absent', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Flexible:
        type: object
        properties:
          a: { type: string }
          b: { type: integer }
    `)
    );
    expect(result).toContain('a?: string;');
    expect(result).toContain('b?: number;');
  });
});

// ── Validation Constraints (3.1-#23-#29) — Tier 2 ─────────────────────────
// Test strategy: no crash + constraint NOT in TypeScript output + base type IS correct.

describe('OpenAPI 3.1 — Validation Constraints, Tier 2 (3.1-#23-#29)', () => {
  // 3.1-#23: minItems
  it('3.1-#23: minItems does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      TagList:
        type: array
        items: { type: string }
        minItems: 1
    `)
    );
    expect(result).toContain('export type TagList = string[];');
    expect(result).not.toContain('minItems');
  });

  // 3.1-#24: maxItems
  it('3.1-#24: maxItems does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      LimitedList:
        type: array
        items: { type: integer }
        maxItems: 10
    `)
    );
    expect(result).toContain('export type LimitedList = number[];');
    expect(result).not.toContain('maxItems');
  });

  // 3.1-#25: minLength
  it('3.1-#25: minLength does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Username:
        type: string
        minLength: 3
    `)
    );
    expect(result).toContain('export type Username = string;');
    expect(result).not.toContain('minLength');
  });

  // 3.1-#26: maxLength
  it('3.1-#26: maxLength does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      ShortName:
        type: string
        maxLength: 50
    `)
    );
    expect(result).toContain('export type ShortName = string;');
    expect(result).not.toContain('maxLength');
  });

  // 3.1-#27: pattern
  it('3.1-#27: pattern does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Email:
        type: string
        pattern: '^[a-zA-Z0-9]+@[a-zA-Z0-9]+\\.[a-zA-Z]{2,}$'
    `)
    );
    expect(result).toContain('export type Email = string;');
    expect(result).not.toContain('pattern');
  });

  // 3.1-#28: minimum
  it('3.1-#28: minimum does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Age:
        type: integer
        minimum: 0
    `)
    );
    expect(result).toContain('export type Age = number;');
    expect(result).not.toContain('minimum');
  });

  // 3.1-#29: maximum
  it('3.1-#29: maximum does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Percentage:
        type: integer
        maximum: 100
    `)
    );
    expect(result).toContain('export type Percentage = number;');
    expect(result).not.toContain('maximum');
  });

  it('3.1-#23-#29: multiple constraints together do not crash', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      BoundedString:
        type: string
        minLength: 1
        maxLength: 255
        pattern: "^[a-z]+$"
    `)
    );
    expect(result).toContain('export type BoundedString = string;');
    expect(result).not.toContain('minLength');
    expect(result).not.toContain('maxLength');
    expect(result).not.toContain('pattern');
  });
});

// ── Exclusive Bounds (3.1-#30-#31) — Tier 3 ───────────────────────────────
// 3.1 uses numeric exclusiveMinimum/Maximum; 3.0 uses boolean + minimum/maximum.

describe('OpenAPI 3.1 — Exclusive Bounds, Tier 3 (3.1-#30-#31)', () => {
  // 3.1-#30: exclusiveMinimum
  it('3.1-#30: 3.1 numeric exclusiveMinimum does not crash', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      ExclusiveMin:
        type: number
        exclusiveMinimum: 0
    `)
    );
    expect(result).toContain('export type ExclusiveMin = number;');
  });

  it('3.1-#30: 3.0 boolean exclusiveMinimum does not crash and produces same type', () => {
    const doc: OpenAPIDocument = {
      openapi: '3.0.3',
      info: { title: 'Test', version: '1.0.0' },
      paths: {},
      components: {
        schemas: {
          ExclusiveMin: {
            type: 'number' as const,
            exclusiveMinimum: true,
            minimum: 0,
          },
        },
      },
    };
    const resolver = new RefResolver(doc);
    const result = generateContracts(doc, resolver);
    expect(result).toContain('export type ExclusiveMin = number;');
  });

  // 3.1-#31: exclusiveMaximum
  it('3.1-#31: 3.1 numeric exclusiveMaximum does not crash', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      ExclusiveMax:
        type: number
        exclusiveMaximum: 100
    `)
    );
    expect(result).toContain('export type ExclusiveMax = number;');
  });

  it('3.1-#31: 3.0 boolean exclusiveMaximum does not crash and produces same type', () => {
    const doc: OpenAPIDocument = {
      openapi: '3.0.3',
      info: { title: 'Test', version: '1.0.0' },
      paths: {},
      components: {
        schemas: {
          ExclusiveMax: {
            type: 'number' as const,
            exclusiveMaximum: true,
            maximum: 100,
          },
        },
      },
    };
    const resolver = new RefResolver(doc);
    const result = generateContracts(doc, resolver);
    expect(result).toContain('export type ExclusiveMax = number;');
  });

  // Both bounds together produce same output across versions
  it('3.1-#30-#31: both exclusive bounds produce same base type in 3.1 and 3.0', () => {
    const result31 = generateFromYaml(
      BASE_SPEC(`
      Bounded:
        type: integer
        exclusiveMinimum: 1
        exclusiveMaximum: 100
    `)
    );

    const doc30: OpenAPIDocument = {
      openapi: '3.0.3',
      info: { title: 'Test', version: '1.0.0' },
      paths: {},
      components: {
        schemas: {
          Bounded: {
            type: 'integer' as const,
            exclusiveMinimum: true,
            minimum: 1,
            exclusiveMaximum: true,
            maximum: 100,
          },
        },
      },
    };
    const resolver = new RefResolver(doc30);
    const result30 = generateContracts(doc30, resolver);

    expect(result31).toContain('export type Bounded = number;');
    expect(result30).toContain('export type Bounded = number;');
  });
});

// ── Examples (3.1-#32) — Tier 3 ───────────────────────────────────────────
// 3.1 uses `examples` array; 3.0 uses `example` single value.

describe('OpenAPI 3.1 — Examples, Tier 3 (3.1-#32)', () => {
  it('3.1-#32: 3.1 examples array does not crash and is not emitted in output', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Color:
        type: string
        examples:
          - red
          - green
          - blue
    `)
    );
    expect(result).toContain('export type Color = string;');
    expect(result).not.toContain("'red' | 'green'");
  });

  it('3.1-#32: 3.0 example single value does not crash and produces same type', () => {
    const doc: OpenAPIDocument = {
      openapi: '3.0.3',
      info: { title: 'Test', version: '1.0.0' },
      paths: {},
      components: {
        schemas: {
          Color: {
            type: 'string' as const,
            example: 'red',
          },
        },
      },
    };
    const resolver = new RefResolver(doc);
    const result = generateContracts(doc, resolver);
    expect(result).toContain('export type Color = string;');
  });

  it('3.1-#32: both 3.1 examples and 3.0 example produce same TypeScript type', () => {
    const result31 = generateFromYaml(
      BASE_SPEC(`
      Name:
        type: string
        examples:
          - Alice
          - Bob
    `)
    );

    const doc30: OpenAPIDocument = {
      openapi: '3.0.3',
      info: { title: 'Test', version: '1.0.0' },
      paths: {},
      components: {
        schemas: {
          Name: {
            type: 'string' as const,
            example: 'Alice',
          },
        },
      },
    };
    const resolver = new RefResolver(doc30);
    const result30 = generateContracts(doc30, resolver);

    expect(result31).toContain('export type Name = string;');
    expect(result30).toContain('export type Name = string;');
  });

  it('3.1-#32: examples on object schema do not crash', () => {
    const result = generateFromYaml(
      BASE_SPEC(`
      Product:
        type: object
        properties:
          name: { type: string }
          price: { type: number }
        examples:
          - name: Widget
            price: 9.99
    `)
    );
    expect(result).toContain('export type Product = {');
    expect(result).toContain('name?: string;');
    expect(result).toContain('price?: number;');
  });
});
